...
# - Every match is also labelled `mailing-lists`, the way The Feed labels
#   `newsletters`, so the web UI can show all list traffic in one view.
# - The `mailing-lists` label and the folders must already exist in Proton
#   (folders nest as "Parent/Child"; folders and labels share one namespace,
#   so the label cannot be called `lists` next to a `Lists` folder).
#
